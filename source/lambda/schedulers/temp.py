...
INF_STARTING_CLUSTERS = "Starting clusters {} in region {}"
INF_STOPPED_CLUSTERS = "Stopping clusters {} in region {}"
INF_AZ_UPSCALED_INSTANCES = "AZ Upscaling instances {} in region {}"
...
class InstanceScheduler:
    ...
    def __init__(self, service, scheduler_configuration):
        ...
        self._scheduler_start_list_cluster = []
        self._scheduler_stop_list_cluster = []
        self._schedule_resize_list = []
        self._scheduler_az_upscale_list = []
    ...
    def _process_account(self, account):
        ...
        started_instances = {}
        started_instances_cluster = {}
        stopped_instances = {}
        stopped_instances_cluster = {}
        resized_instances = {}
        az_upscaled_instances = {}
        ...
        for region in self._regions:
            ...
            for instance in self._scheduled_instances_in_region(account, region):
                ...
                # if enforced check the actual state with the desired state enforcing the schedule state
                    ...
                    if (instance.is_running and desired_state == InstanceSchedule.STATE_STOPPED) or (
                        ...
                        self._process_new_desired_state(account, region, instance, desired_state, desired_type, last_desired_state,
                                                        instance_schedule.retain_running)
                    elif last_desired_state == InstanceSchedule.STATE_STARTED_FOR_AZ_UPSCALE:
                        self._process_new_desired_state(account, region, instance, desired_state, desired_type, last_desired_state,
                                                    instance_schedule.retain_running)
            ...
            if len(self._scheduler_start_list_cluster) > 0:
                started_instances_cluster[region] = [{i.id: {"schedule": i.schedule_name}} for i in self._scheduler_start_list_cluster]
            if len(self._scheduler_stop_list_cluster):
                stopped_instances_cluster[region] = [{i.id: {"schedule": i.schedule_name}} for i in self._scheduler_stop_list_cluster]            
            if len(self._scheduler_az_upscale_list):
                az_upscaled_instances[region] = [{i.id: {"schedule": i.schedule_name}} for i in self._scheduler_az_upscale_list]
        ...
        if self._service.allow_az_upscale:
            result["az_upscaled"] = az_upscaled_instances        
        
        try:
            if self._service._config.schedule_clusters:
                result["started_cluster": started_instances_cluster]
                result["stopped_cluster": stopped_instances_cluster]
        except Exception as ex:
            self._logger.info("Skipping cluster results for service {}".format(self._service.service_name))
        ...
        if last_desired_state == InstanceSchedule.STATE_RETAIN_RUNNING:
            ...
            if desired_state == InstanceSchedule.STATE_RUNNING:
                if not instance.is_running:
                    ...
                    if not instance.is_cluster:
                        self._scheduler_start_list.append(instance)
                    else:
                        self._scheduler_start_list_cluster.append(instance)
                ...
                # instance already running with desired state of running
                    ...
                    if last_desired_state == InstanceSchedule.STATE_STOPPED:
                        ...
                        self._scheduler_az_upscale_list.append(instance)
                ...
                if instance.is_running:
                    ...
                    if not instance.is_cluster:
                        ...
                        self._scheduler_stop_list_cluster.append(instance)
    ...
    def _start_and_stop_instances(self, account, region):
        ...
        if len(self._scheduler_az_upscale_list) > 0:
            self._logger.info(INF_AZ_UPSCALED_INSTANCES, ", ".join([i.instance_str for i in self._scheduler_az_upscale_list]),
                              region)
            for inst_id, state in self._service.az_upscale_instances({
                schedulers.PARAM_SESSION: account.session,
                schedulers.PARAM_ACCOUNT: account.name,
                schedulers.PARAM_ROLE: account.role,
                schedulers.PARAM_REGION: region,
                schedulers.PARAM_TRACE: self._configuration.trace,
                schedulers.PARAM_AZ_UPSCALED_INSTANCES: self._scheduler_az_upscale_list,
                schedulers.PARAM_LOGGER: self._logger,
                schedulers.PARAM_CONTEXT: self._context,
                schedulers.PARAM_STACK: self._stack_name,
                schedulers.PARAM_CONFIG: self._scheduler_configuration
            }):
                # set state based on start of upscale action
                self._instance_states.set_instance_state(inst_id, state)

        if len(self._scheduler_start_list_cluster) > 0:
            self._logger.info(INF_STARTING_CLUSTERS, ", ".join([i.instance_str for i in self._scheduler_start_list_cluster]),
                              region)
            ...
            for inst_id, state in self._service.start_instances({
                ...
                schedulers.PARAM_REGION: region,
                schedulers.PARAM_TRACE: self._configuration.trace,
                schedulers.PARAM_STARTED_INSTANCES: self._scheduler_start_list_cluster,
                schedulers.PARAM_LOGGER: self._logger,
                schedulers.PARAM_CONTEXT: self._context,
                schedulers.PARAM_STACK: self._stack_name,
                schedulers.PARAM_CONFIG: self._scheduler_configuration
        ...
        if len(self._scheduler_stop_list_cluster) > 0:
            self._logger.info(INF_STOPPED_CLUSTERS, ", ".join([i.instance_str for i in self._scheduler_stop_list_cluster]),
                              region)
            for inst_id, state in self._service.stop_instances({
                schedulers.PARAM_SESSION: account.session,
                schedulers.PARAM_ACCOUNT: account.name,
                ...
                schedulers.PARAM_REGION: region,
                schedulers.PARAM_TRACE: self._configuration.trace,
                schedulers.PARAM_STOPPED_INSTANCES: self._scheduler_stop_list_cluster,
                schedulers.PARAM_LOGGER: self._logger,
                schedulers.PARAM_CONTEXT: self._context,
                schedulers.PARAM_STACK: self._stack_name,
                schedulers.PARAM_CONFIG: self._scheduler_configuration
